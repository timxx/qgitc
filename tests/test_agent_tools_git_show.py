...
class TestGitShowTool(unittest.TestCase):
    ...
    @patch("qgitc.agent.tools.git_show.runGit")
    def test_success(self, mock_runGit):
        mock_runGit.return_value = (True, "commit abc123\nAuthor: Test\n\nSome message")
        result = self.tool.execute({"rev": "abc123"}, _make_context())
        self.assertFalse(result.is_error)
        self.assertIn("abc123", result.content)
        mock_runGit.assert_called_once_with("/fake/repo", ["show", "abc123"])

    @patch("qgitc.agent.tools.git_show.runGit")
    def test_failure(self, mock_runGit):
        mock_runGit.return_value = (False, "fatal: bad object abc")
...
class TestGitShowFileTool(unittest.TestCase):
    ...
    @patch("qgitc.agent.tools.git_show_file.runGit")
    def test_success_full_file(self, mock_runGit):
        mock_runGit.return_value = (True, "line1\nline2\nline3\nline4")
    ...
    @patch("qgitc.agent.tools.git_show_file.runGit")
    def test_success_with_line_range(self, mock_runGit):
        mock_runGit.return_value = (True, "line1\nline2\nline3\nline4")
        result = self.tool.execute(
    ...
    @patch("qgitc.agent.tools.git_show_file.runGit")
    def test_success_with_start_line_only(self, mock_runGit):
        mock_runGit.return_value = (True, "line1\nline2\nline3\nline4")
        result = self.tool.execute(
    ...
    @patch("qgitc.agent.tools.git_show_file.runGit")
    def test_success_with_end_line_only(self, mock_runGit):
        mock_runGit.return_value = (True, "line1\nline2\nline3\nline4")
        result = self.tool.execute(
    ...
    @patch("qgitc.agent.tools.git_show_file.runGit")
    def test_failure(self, mock_runGit):
        mock_runGit.return_value = (False, "fatal: path 'missing' does not exist")
        result = self.tool.execute(
...
class TestGitShowIndexFileTool(unittest.TestCase):
    ...
    @patch("qgitc.agent.tools.git_show_index_file.runGit")
    def test_success_full_file(self, mock_runGit):
        mock_runGit.return_value = (True, "idx1\nidx2\nidx3")
    ...
    @patch("qgitc.agent.tools.git_show_index_file.runGit")
    def test_failure(self, mock_runGit):
        mock_runGit.return_value = (False, "fatal: path 'x' does not exist in the index")
...
class TestGitBlameTool(unittest.TestCase):
    ...
    @patch("qgitc.agent.tools.git_blame.runGit")
    def test_basic_blame(self, mock_runGit):
        mock_runGit.return_value = (True, "abc123 (Author 2024-01-01 1) line1")
        result = self.tool.execute({"path": "foo.py"}, _make_context())
        self.assertFalse(result.is_error)
        self.assertIn("abc123", result.content)
        mock_runGit.assert_called_once_with(
    ...
    @patch("qgitc.agent.tools.git_blame.runGit")
    def test_blame_without_whitespace_ignore(self, mock_runGit):
        mock_runGit.return_value = (True, "abc123 (Author 2024-01-01 1) line1")
        result = self.tool.execute(
    ...
    @patch("qgitc.agent.tools.git_blame.runGit")
    def test_blame_with_rev(self, mock_runGit):
        mock_runGit.return_value = (True, "abc123 (Author 2024-01-01 1) line1")
        result = self.tool.execute(
    ...
    @patch("qgitc.agent.tools.git_blame.runGit")
    def test_blame_with_start_and_end_line(self, mock_runGit):
        mock_runGit.return_value = (True, "abc123 (Author 2024-01-01 5) line5")
        result = self.tool.execute(
    ...
    @patch("qgitc.agent.tools.git_blame.runGit")
    def test_blame_with_start_line_only(self, mock_runGit):
        mock_runGit.return_value = (True, "abc123 (Author 2024-01-01 5) line5")
        result = self.tool.execute(
    ...
    @patch("qgitc.agent.tools.git_blame.runGit")
    def test_blame_with_end_line_only(self, mock_runGit):
        mock_runGit.return_value = (True, "abc123 (Author 2024-01-01 1) line1")
        result = self.tool.execute(
    ...
    @patch("qgitc.agent.tools.git_blame.runGit")
    def test_blame_empty_output(self, mock_runGit):
    ...
    @patch("qgitc.agent.tools.git_blame.runGit")
    def test_blame_failure(self, mock_runGit):
        mock_runGit.return_value = (False, "fatal: no such path 'x' in HEAD")
        result = self.tool.execute({"path": "x"}, _make_context())
        self.assertTrue(result.is_error)
        self.assertIn("fatal", result.content)

    @patch("qgitc.agent.tools.git_blame.runGit")
    def test_blame_with_rev_and_line_range(self, mock_runGit):
        mock_runGit.return_value = (True, "abc (A 2024-01-01 2) l2")
        result = self.tool.execute(
